...
public class CSVReaderTest {
    ...
    void whenFilterTwoColumns(@TempDir Path folder) throws Exception {
        String data = String.join(
                ...
                "name;age;last_mame;education",
                "Tom;20;Smith;Bachelor",
                "Jack;25;Johnson;Undergraduate",
                "William;30;Brown;Secondary special"
        );
        File file = folder.resolve("source1.csv").toFile();
        File target = folder.resolve("target1.csv").toFile();
    ...
    void whenFilterThreeColumns(@TempDir Path folder) throws Exception {
        ...
        File file = folder.resolve("source2.csv").toFile();
        File target = folder.resolve("target2.csv").toFile();
        ArgsName argsName = ArgsName.of(new String[]{
                "-path=" + file.getAbsolutePath(), "-delimiter=,",
